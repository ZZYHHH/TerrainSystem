	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float indexLayer4 = floor((index.a * 15));
				...
				float4 colorLayer4 = GetColorByIndex(indexLayer4, lodLevel, i.worldPos.xz);

				//混合因子，其中r通道为第一层贴图所占权重，g通道为第二层贴图所占权重，b通道为第三层贴图所占权重
				float4 blend = tex2D (_BlendTex, i.uv.xy);
				half4 albedo = (colorLayer1 * blend.r + colorLayer2 * blend.g + colorLayer3 * blend.b+colorLayer4*blend.a)/(blend.r+blend.g+blend.b+blend.a);
